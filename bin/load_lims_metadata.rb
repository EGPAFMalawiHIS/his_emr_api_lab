...
CSV.open(Rails.root.join($ARGV[0]), headers: :first_row) do |csv|
  ...
  csv.each do |row|
    TEST_TYPE_CONCEPT_NAME = row[1]
    sample_type_name = row[0]

    ActiveRecord::Base.transaction do
      test_type = test_types[TEST_TYPE_CONCEPT_NAME] || create_test_type(TEST_TYPE_CONCEPT_NAME)
      create_sample_type(sample_type_name, test_type)

      test_types[TEST_TYPE_CONCEPT_NAME] ||= test_type
    end

    puts "Created test #{TEST_TYPE_CONCEPT_NAME} <--< #{sample_type_name}"
  rescue StandardError => e
    puts "Error: #{TEST_TYPE_CONCEPT_NAME}: #{e}"
  end
end
